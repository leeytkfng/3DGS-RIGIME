...
DTU는 궤도형 rig이므로 overlap과 baseline이 강하게 결합되어 있다는 점은 한계다 — RE10K/DL3DV 같은 경로형(trajectory) 촬영에서는 결합 강도가 다를 수 있어 동일 분석의 반복이 후속 과제로 남는다.
...
\chk{두 원인이 섞여 있을 수 있다 — (i) 관측 부족(적절히 배치됐으나 카메라 수가 적어 소수 view에서만 보임) 대 (ii) 배치 실패(random-sphere fallback 점이 애초에 어떤 카메라 절두체에도 안 잡힘). 어떤 view에도 투영되지 않는 Gaussian 비율을 별도 측정해 분리하는 작업이 남아 있으며, 결과에 따라 본 절의 주장이 ``sparse-view 일반의 메커니즘''인지 ``triangulation 실패 시 random 초기화의 한계''인지가 갈린다.}
...
\chk{\textbf{표본 해상도의 한계}: 예산 스냅샷이 1/10/60/300\,s 4개뿐이라 ``정점 시점''과 ``하강 기울기'' 추정 모두 해상도가 거칠다 — 특히 위 하강폭 결과는 12-view Vanilla3DGS의 60\,s 지점 비단조성(\S\ref{sec:results})이 4점 표본에 어떻게 걸리느냐에 따라 만들어졌을 수 있어, ``하강이 실제로 view 수에 따라 커진다''와 ``60\,s 근방의 일시적 하락이 우연히 잡혔다''를 이 표본만으로는 구분할 수 없다. 더 촘촘한 스냅샷(예: 5/15/30/60/120/300\,s) 재실행이 필요하다.}
...
\caption{C2 depth-noise sensitivity, DTU 8-scan, budget=300\,s. 왼쪽 두 패널은 개별 노이즈 $\sigma$ 축, 오른쪽 두 패널은 전역 스케일 편향 $s$ 축. PSNR 패널은 4개 조건 모두 예측대로 하락($\sigma$)/단조 증가($s$)하지만, Gaussian 수 패널은 H1이 예측한 증가가 아니라 하락 또는 평탄이다. 오차 막대는 scene cluster bootstrap 95\% CI.}
...
\chk{$s$축 탐색 범위 확장($s \in \{1.15, 1.2, 1.3\}$)과 Gaussian growth/prune 카운트 분리 계측이 다음 단계로 남아 있다.}
