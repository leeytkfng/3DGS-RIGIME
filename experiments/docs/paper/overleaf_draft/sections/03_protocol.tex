...
본 실험은 세 통제 축(view 수 $\times$ overlap $\times$ 계산 예산)을 완전교차하며, Regime Map(\S\ref{sec:results}) $\to$ 메커니즘 증거(\S\ref{sec:failure}) $\to$ 통제 검증(\S\ref{sec:c1b}) $\to$ 가이드라인(\S\ref{sec:guideline})의 순서로 서술한다.
