...
17. 🔄 **[진행 중]** C1-a 본 실험 착수 — 2026-08-13, `run_re10k_c1a_main.py`. RE10K 30 scene × view_count{2,4,8,12} × budget{1,10,60,300}s × {MVSplat, Vanilla3DGS×seed2, FSGS×seed2}, overlap_level 축은 이번 착수에서 제외(selector 3-scene pilot 검증만 됨, §16 참고). 백그라운드 실행 중(nohup, ~40시간 예상), 진행 상황은 Artifact 대시보드로 확인 가능. 완료되면 §5.12 τ(0.5/1.4dB)·scene-cluster bootstrap 적용해 최종 regime map 산출
    - [x] **FSGS 300초 budget 붕괴 버그 발견·수정** — 2026-08-14, 50/120 combo 시점 중간 점검 중 발견. `opt.iterations`(=10,000, FSGS 원본 기본값) 초과 시 `optimizer.step()`이 멈추는데 `opacity_reset_interval`(3,000)은 상한 없이 계속 발동 — 300s budget이 4~5만 iteration까지 가는 우리 환경에서 opacity가 반복 초기화되며 회복 못 하고 붕괴(예: 27.5dB→14.4dB). `fsgs_runner.py` 수정(optimizer 항상 step, reset은 `step<densify_until_iter`로 제한) 후 같은 조건 재검증(22.00→22.46dB, 정상 개선). 오염된 FSGS 로그 100개 삭제 후 재실행(Vanilla3DGS/MVSplat 101+50개는 보존, 재개 로직으로 FSGS만 다시 도는 것 실측 확인). 상세: `daily_report_2026-08-14.md` §1
...
19. ~~논문 초안(Overleaf용 LaTeX) 작성~~ ✅ (2026-08-13) — `experiments/docs/paper/overleaf_draft/main.tex`. Intro/Related Work/Protocol/한계는 지금까지 확정된 내용으로 채움, 실패분석은 geometry confound·observation starvation 실측 결과를 이미 반영. Results/Guideline/C2/H1·H2·H3 표는 `\todo{}`로 명시적으로 비워둠(본 실험 결과 대기). 인용 7개(3DGS/pixelSplat/MVSplat/DepthSplat/FSGS/RE10K/DL3DV) 전부 웹서치로 서지사항 확인, cite-bibitem 키 매칭 검증 완료. **남은 일**: C1-a 결과 나오면 §4(Regime Map) 채우기, C1-b/C2 완료 후 나머지 `\todo{}` 채우기
   - [x] **사용자가 직접 프로토콜 절·OOD confound 절·인용 8개(Diff3R/ForeSplat/SplatFormer/Omni-Scene/MVSNet/SfM/gsplat/DTU) 추가** — 2026-08-13. 전부 웹서치로 저자·venue 대조 검증(Diff3R 이름 오타 Hlad→Hladk 수정, ForeSplat 11인 전체 저자 확인). **SplatFormer 인용 1건은 구체 주장이 원문에서 직접 확인 안 됨** — `\chk{}`로 표시, 본문 확인 필요
   - [x] **실제 그림 2개 생성**(placeholder 아님) — `experiments/scripts/analysis/make_paper_figures.py` 신규. observation starvation 비율 bar chart, geometry confound 산점도(raw/log-baseline 상관 실측값과 정확히 일치, 0.952/-0.989) — 문서 논지에 이미 있던 실측 수치를 그림으로 재현. 한글 폰트 없어 라벨은 영문으로
   - [x] **나머지 figure 자리 확보**: 프로토콜 개요 다이어그램(제 추천: 모델 아키텍처 파이프라인은 불필요, 통제축+서사사슬 개요만), Regime Map heatmap, Pareto frontier, 정성적 렌더 비교 — 전부 `\todo{}`로 자리와 생성 방법 명시
20. ~~학습 계획(study plan) 문서화~~ ✅ (2026-08-13) — `experiments/docs/paper/study_plan.md`. 완료: 야코비안/공분산, Vanilla 3DGS 기초. 다음 5단계 상세 계획(읽을 자료/대조 코드/이해 목표/확인 질문 포함): ①densification 메커니즘(오늘 발견과 직결, 최우선 추천) ②3DGS 원논문 ADC 절 ③FSGS ④MVSplat ⑤DepthSplat
21. ~~overlap_level 축을 러너에 연결~~ ✅ (2026-08-14) — `vanilla_3dgs_runner.py`/`fsgs_runner.py`/`mvsplat_re10k_runner.py` 세 개 다 `--overlap-level {high,low}` 추가(미지정 시 기존 동작 그대로, 하위호환). `re10k_overlap_candidates.json`(어제 30-scene 전체 검증한 selector 출력)에서 view 선택. 경로 충돌 위험도 같이 고침(checkpoint/log/colmap-workdir 파일명에 overlap suffix 없어서 미래 high/low 실행이 지금 도는 view_count-only 데이터를 덮어쓸 뻔함). 세 러너 전부 짧은 budget으로 실제 실행 검증(로그 파일명에 `_high`/`_low` 정상 반영). **남은 일**: `run_re10k_c1a_main.py`에 이 축을 실제로 추가해서 실행하는 건 현재 진행 중인 view_count-only 본 실험 완료 후로 미룸(GPU 하나뿐이라 순차)
22. ~~ReSplat 정독 및 스코프 결정~~ ✅ (2026-08-16) — 사용자가 arXiv 2510.08575 직접 정독, 정리 노트: `experiments/docs/reference/paper_notes/resplat.md`. 핵심 발견: ReSplat 부록 Tab.~S1이 view 수 늘릴 때 촬영 범위·test view를 함께 바꿨다고 자인(view 수-overlap 교란) — 본 연구 문제의식이 최신 SOTA에서도 미해소라는 직접 증거. 또한 view 8→32에서 자신-3DGS PSNR 격차가 단조 축소(+2.76→+1.63→+0.44dB, 32-view는 LPIPS 역전)해 우리 "역전 경계 존재" 가설을 경쟁 논문 데이터가 지지. **결정**: ① Related Work 인용 확대 반영 완료(`main.tex` 커밋, 위 두 사실 포함) ② 메인 Regime Map에는 미포함(다른 패러다임 축, 전체 재실행 필요해 일정상 불가) ③ Limitations에 "recurrent 갱신 계열은 별도 패러다임으로 스코프 제외, 후속 과제" 문장 추가 완료. **미결(조건부)**: 역전 경계 근처(4/8/12-view × 10/60s)만 소규모 ReSplat 확장 비교 — seed 기반 view split 강제 주입 가능 여부, RE10K 체크포인트가 2-view 학습이라 8/12-view는 MVSplat과 동일하게 분포 밖이라는 점, 우리 test view·해상도·pose 규칙과의 정렬 가능 여부 세 가지 확인 후 착수 여부 결정(다음 세션 과제)
