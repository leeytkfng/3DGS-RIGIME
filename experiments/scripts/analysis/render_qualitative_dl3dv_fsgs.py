#!/usr/bin/env python3
"""정성적 비교 그림(DL3DV)의 FSGS 렌더 — `fsgs` conda env에서 실행. 재실행 없음."""
...
def main() -> int:
    ...
    traj_path = c1a_root / "fsgs_runs" / scene / "logs" / f"dl3dv_{scene}_c1a_FSGS_{view_count}view_seed{seed}.json"
    ...
    gaussians = GaussianModel(SimpleNamespace(sh_degree=3))
