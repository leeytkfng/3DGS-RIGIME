...
- **2026-08-15 init 경로 구현 중 발견한 스케일 버그와 교정**: `--init-source depth_backprojection` 구현 후 DTU scan1 4-view로 실제 back-projection해보니, 여러 view의 point cloud를 합친 중심이 실제 scene 중심(카메라 기하로 독립 추정한 값)에서 scene radius의 2배 넘게 벗어났다(실측: 거리 515.6 vs radius 213.0). 원인은 monocular metric depth 모델(Depth Anything V2 Metric-Indoor)이 DTU 같은 근접(macro) 촬영에는 학습 분포 밖이라 절대 스케일이 view마다 제각각 크게 틀렸기 때문(실측 원출력 0.5~3.9 범위, 실제 카메라-scene중심 거리는 ~608). R/t 변환 자체는 별도로 검증해 정확함을 확인(알려진 카메라-중심 거리를 상수 depth로 강제 주고 back-project하면 scene 중심 15 이내로 정확히 맞음). **교정**: pose-given이라 카메라 위치는 정확히 알고 있으므로, view별 median depth를 "카메라-scene중심 거리"에 맞춰 재조정하는 스케일 보정을 추가(`precompute_depth_maps.py::calibrate_depth`) — σ=0/scale_bias=1.0(교란 없음) 기준선을 실제로 쓸만한 depth로 만들기 위한 전처리이며, C2가 통제하는 sigma/scale_bias 교란과는 별개다. 교정 후 중심 거리 145.7(< radius 213)로 개선, 같은 조건 학습 스모크에서 test PSNR 5.57→9.08dB(1s), SSIM 0.042→0.39로 뚜렷이 개선 확인.
